...
public class MainMenu implements Handler{
    ...
    private Direction pointerPos = Direction.LEFT;
    private Game game;
    ...
    public void handlePressed(KeyboardEvent key) {
        ...
        switch (key.getKey()) {
            case KeyboardEvent.KEY_LEFT:
                if (showPointer) {
                    pointerPos = Direction.LEFT;
                    this.pointer.delete();
                    pointer = new Picture(270, 402, "res/Pointer.png");
                    pointer.draw();}
                break;
            case KeyboardEvent.KEY_RIGHT:
                if (showPointer) {
                    pointerPos = Direction.RIGHT;
                    this.pointer.delete();
                    pointer = new Picture(545, 402, "res/Pointer.png");
                    pointer.draw();}
                break;
            case KeyboardEvent.KEY_SPACE:
                if (this.pointerPos == Direction.LEFT) {
                    showPointer = false;
                    start();
                } else {
                    System.exit(0);
